  ...
  const renderWebcam = () => (
    <>
      <Webcam
        audio={false}
        ref={webcamRef}
        className={!isLoaded ? "video" : "video frame"}
      />
  ...
  return (
    <div className="container">
      <header className="header">
        <h1>Face Emoji Mask</h1>
        &nbsp;
        <label className="switch">
          <input
            type="checkbox"
            checked={webcamEnabled}
            onClick={() => setWebcamEnabled((previous) => !previous)}
          />
      ...
      <main className="main">{webcamEnabled && renderWebcam()}</main>

      <footer>
        <p>
          Made by Dennis Wang (王定) -{" "}
          <a
            href="https://github.com/devmaster518/tfjs-face-emoji-mask"
            target="_blank"
            rel="noreferrer"
          >
            GitHub Source
